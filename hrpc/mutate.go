...
func TimestampUint64(ts uint64) func(Call) error {
	return func(o Call) error {
		...
		if m.timestamp > MaxTimestamp { // TODO claire test
			return fmt.Errorf("timestamp is greater than max: (%d > %d)", m.timestamp, MaxTimestamp)
		}
		m.timestamp = ts
		return nil
	}
...
func (m *Mutate) valuesToCellblocks() ([]byte, int32, uint32) {
	...
	var ts uint64
	if m.timestamp >= MaxTimestamp { // TODO claire
		ts = MaxTimestamp
	} else {
		ts = m.timestamp
	}
