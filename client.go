...
func (c *Client) SendRPC(rpc hrpc.Call) (proto.Message, error) {
	// On the first SendRPC, a goroutine must be manually kicked off for the
	// meta or admin region client, depending on the type of client.
	if c.clientType == AdminClient && c.adminClient == nil && !c.adminRegionInfo.IsUnavailable() {
		c.adminRegionInfo.MarkUnavailable()
		go c.reestablishRegion(c.adminRegionInfo)
	}
	if c.clientType == StandardClient && c.metaClient == nil && !c.metaRegionInfo.IsUnavailable() {
	...
	if reg != nil {
		...
		if ch == nil {
			...
			if _, ok := res.Error.(region.RetryableError); ok {
				...
				// considered dead. Mark the region as unavailable, if this
				// was the first goroutine to do so start a goroutine to
				// reestablish a connection, and recurse, which will result in
				// blocking until the region is available again.
				first := reg.MarkUnavailable()
				if first {
					go c.reestablishRegion(reg)
				}
		...
		for {
			...
			if reg2 != nil {
				...
				// up. Recurse to use the version in the cache.
			...
			// Recurse, which will result in blocking until region is available
			return c.SendRPC(rpc)
		}
	}
	// This should never be reached.
	return nil, errors.New("Internal gohbase error")
}
...
func (c *Client) locateRegion(ctx context.Context, table, key []byte) (*regioninfo.Info, string, uint16, error) {
	...
	gresp := resp.(*pb.GetResponse)
	for _, cell := range gresp.Result.Cell {
...
var newRegion = func(ret chan newRegResult, clientType region.ClientType, host string, port uint16, queueSize int, queueTimeout time.Duration) {
